...
import math
import time
#import matplotlib.pyplot as plt #uncomment to use plots
import os

#Math for rotation - функции для получения углов вращения по данным акселерометров
def dist(a,b):
...
def mpu_activate(muxAddr, muxMpu, mpuAddr): #активация датчиков
...
def read_register_mpu(mpuAddr, mpuRegister): #считываются значения из регистров
    high = bus.read_byte_data(mpuAddr, mpuRegister)
    low = bus.read_byte_data(mpuAddr, mpuRegister+1)
    val = (high << 8) + low
    if (val >= 0x8000):  #преобразование значений, представленных в дополнительном коде
        return -((65535 - val) + 1)
    else:
        return val
def mpu_data_acquire(muxAddr, muxMpu, mpuAddr): #получение данных из регистров датчика
    bus.write_byte_data(muxAddr, 0, mux_channels[muxMpu]) #выбор канала мультиплексора
    gyro_xout = read_register_mpu(mpuAddr, 0x43)
    gyro_yout = read_register_mpu(mpuAddr, 0x45)
    gyro_zout = read_register_mpu(mpuAddr, 0x47)
    gyro_xout_scaled = gyro_xout/131
    gyro_yout_scaled = gyro_yout/131
    gyro_zout_scaled = gyro_zout/131
    accel_xout = read_register_mpu(mpuAddr, 0x3b)
    accel_yout = read_register_mpu(mpuAddr, 0x3d)
    accel_zout = read_register_mpu(mpuAddr, 0x3f)
...
#Final rotations
def integral_trap(x, xold, dt): #метод трапеции для интегрирования
    int_trap = (x + xold)*dt/2
    return int_trap
def comp_filter_trap(compf_old, gyro, gyro_old, rot, t): #комплементарный фильтр, использующий метод трапеции для интегрирования
    alpha = 0.05
    compf = (1-alpha) * (compf_old + integral_trap(gyro, gyro_old, t)) + alpha * (rot)
    return compf
    
mpu_address_1 = 0x68 #mpu_address_2 = 0x69
mpu_power_mgmt = 0x6b
...
for MPU in range(MPUs):
    mpu_activate(mux_upp_address, MPU, mpu_address_1)    #activates the MPUs through multiplexer
...
#Uncomment the following to use plots
"""
...
#Asking the user if we should use the code to establish the starting position
...
if start_position:
    ...
    for MPU in range(MPUs): #This code collects data from all the used MPUs
        result = mpu_data_acquire(mux_upp_address, MPU, mpu_address_1)
...
if MPUs == 1:
    ...
    for MPU in range(MPUs):
        ax[MPU//columns,MPU%columns].text(0.5, 0.5, 'MPU '+ str(MPU), fontsize=15, ha='center')
        ax[MPU//columns,MPU%columns].plot(list_at, listcompfx[MPU], color="purple", label="X (CF)")
        ...
        ax[MPU//columns,MPU%columns].legend(fontsize = 10, loc="lower left", frameon=True)
"""

print('Start moving sensors.')

tmp = 1 #tmp = True
while tmp: #while tmp #while tmp < 1000 #бесконечный цикл для получения результатов считывания
...
#Uncomment the following to use plots
    ...
    list_at.append(at)
    """
#Comment the following print functions if console output is not needed

    print("Движение правого плеча: ")
    print("    вращение вперед-назад: " + str((compfx_ac[1] - compfx_0[1]) - (compfy_ac[0] - compfy_0[0]))) #[ 1.1(X) - 2.0(Y)]
    print("    вращение вправо-влево: " + str(-((compfy_ac[1] - compfy_0[1]) + (compfx_ac[0] - compfx_0[0])))) #[ -(1.1(Y) + 2.0(X))]
    print()
    print("Движение правого локтя: ")
    print("    вращение вперед-назад: " + str((compfx_ac[2] - compfx_0[2]) - (compfx_ac[1] - compfx_0[1]))) #[ 1.2(X) - 1.1(X)]
    print()
    print("Движение правой кисти: ")
    print("    вращение вперед-назад: " + str((compfy_ac[3] - compfy_0[3]) - (compfy_ac[2] - compfy_0[2]))) #[ 1.3(Y) - 1.2(Y)]
    print("    вращение вправо-влево: " + str(-((compfx_ac[3] - compfx_0[3]) - (compfx_ac[2] - compfx_0[2])))) #[ -(1.3(X) - 1.2(X))]
