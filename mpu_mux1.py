...
def mpu_data_acquire(mpu_addr, mpu):
    ...
    accel_xout = read_register_mpu(mpu_addr, 0x3b, mpu)
    accel_yout = read_register_mpu(mpu_addr, 0x3d, mpu)
    accel_zout = read_register_mpu(mpu_addr, 0x3f, mpu)
...
while tmp < 1000: #while tmp
    ...
    for MPU in range(MPUs):
        result_s = mpu_data_acquire(mpu_address, MPU)
